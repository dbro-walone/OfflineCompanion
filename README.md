...
- 统一 `PetEvent` 行为运行时、活动动作生命周期、4 秒互动会话、可注入随机源的空闲动作调度，以及 Drop/Edge/Thrown/Landing 拖放闭环；
- 动作包的触发器、权重、冷却和独立图集几何真实参与运行，默认办公动作包会自然播放 `idle.thinking`；
- 扩展包页面支持角色切换、动作包启用/禁用与即时预览、非默认包删除、校验警告和安全回退；
- 设置可即时控制空闲动作、有限靠近、鼠标观察、互动强度、提醒跟随与减少动态效果。
